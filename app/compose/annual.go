...
func Annual(cfg config.Config, tpls []string) (page.Modules, error) {
	...
	return page.Modules{{
		...
		Body: map[string]interface{}{
			...
			"Extra": header.Items{header.NewTextItem("Future").RefText("Future")}.
...
func extra2(ctrc, sel1, sel2 bool, week *cal.Week, idxPage int) header.Items {
	...
	if idxPage > 0 {
		...
		items = append(items, header.NewCellItem("Collections").Refer("Notes Index"+suffix).Selected(sel2))
	} else {
		items = append(items, header.NewCellItem("Collections").Refer("Notes Index").Selected(sel2))
	}
